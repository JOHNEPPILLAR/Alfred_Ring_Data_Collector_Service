...
async function saveRecordings(camera) {
  ...
  await Promise.all(
    eventsWithRecordings.map(async (eventWithRecording) => {
      ...
      const query = {
        filename: `${eventWithRecording.created_at}-${eventWithRecording.ding_id_str}.mp4`,
      };
      ...
      await this._saveStreamToDB(
        dbConnection,
        'media',
        `${eventWithRecording.created_at}-${eventWithRecording.ding_id_str}.mp4`,
        transcodedUrl,
      );

      this.logger.info('Saved motion event');
